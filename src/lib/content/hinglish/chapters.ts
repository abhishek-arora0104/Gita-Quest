...
function quiz(seed: HinglishSeed): Chapter["quiz"] {
  const easy: QuizSeed[] = [
    q(`${seed.title} chapter ka mukhya vishay kya hai?`, [seed.focus, "Paisa kamaana", "Doosron ko haraana", "Gyaan se bachna"], 0, `Is chapter ka kendr ${seed.focus} hai.`),
    q(`Is chapter ka Sanskrit naam kya hai?`, [seed.sanskritName, "Karma Yoga", "Sankhya Yoga", "Dhyana Yoga"], 0, `Is chapter ka naam ${seed.sanskritName} hai.`),
    ...
    q("Is chapter mein kaun si saadhna upyogi batayi gayi hai?", [seed.practice, "Lagataar shikayat", "Aalasy", "Dikhawa"], 0, `${seed.practice} is chapter ki seekh ko jeevan mein laati hai.`),
    q("Yeh chapter kis baat se savdhaan karta hai?", [seed.warning, "Sachcha prayas", "Karuna", "Vinamrata"], 0, `${seed.warning} mann ko satya se door kar sakta hai.`),
    q("Is chapter ka result kis or le jaata hai?", [seed.result, "Aur adhik bhrm", "Lobh", "Ashanti"], 0, `Yeh shiksha ${seed.result} ki or le jaati hai.`),
    q("Vidyarthi ko kya yaad rakhna chahiye?", [seed.remembrance, "Kewal result mayne rakhte hain", "Ahankar hi sarvopari hai", "Doosron ka koi mulya nahi"], 0, seed.remembrance),
    ...
    q("Is chapter ki shiksha kis ke liye upyogi hai?", ["Har sadhak aur vidyarthi ke liye", "Kewal yoddhon ke liye", "Kewal rajaon ke liye", "Kisi ke liye nahi"], 0, "Gita ki shiksha aaj ke jeevan mein bhi lagoo hoti hai."),
    q("Chapter ko samajhne ka sahi tarika kya hai?", ["Shant mann se padhna aur jeevan mein lagoo karna", "Jaldi-jaldi chhod dena", "Sirf shabd yaad karna", "Doosron se tulna karna"], 0, "Gita kewal padhne ke liye nahi, jeene ke liye hai."),
  ];

  const medium: QuizSeed[] = [
    q("Is chapter ko daily jeevan mein kaise lagoo kiya ja sakta hai?", [`${seed.practice} ko chhote-chhote kaamon mein apnaakar`, "Har kathin nirnay se bhaagkar", "Kewal prashansa dhundhkar", "Doosron ko dosh dekar"], 0, "Gita ki seekh chhote daily nirnayon mein dikhai deti hai."),
    q("Kaun sa vyakti is chapter ka behtar paalan karta hai?", [`Jo ${seed.focus} ko samajh kar karma karta hai`, "Jo ahankar se chalta hai", "Jo seekhne se mana karta hai", "Jo anushasan ka mazaak udaata hai"], 0, "Sahi samajh vyavahar mein dikhai deti hai."),
    ...
    q("Is chapter ki ek samanya bhool kya hai?", [`${seed.warning} ko samanya maan lena`, "Nihswarth seva karna", "Ishwar ko yaad rakhna", "Aatm-sanyam rakhna"], 0, "Gita humein sookshm galtiyon ko pehchanna sikhati hai."),
    ...
    q("Yeh chapter humein kis se aage dekhne ko kehta hai?", ["Oopri roop aur ahankar se", "Karuna se", "Seekhne se", "Kartavya se"], 0, "Sachchi drishti baahari lebilon se aage dekhti hai."),
    ...
    q("Bhrm ke samay svasthya feedback kya hai?", ["Gyaan lena aur imandaari se abhyas karna", "Sab jaanne ka naatak karna", "Doosron par hamla karna", "Turant haar maan lena"], 0, "Arjun ki yatra bhi imandaar sawaal se shuru hoti hai."),
  ];

  const hard: QuizSeed[] = [
    q("Is chapter ki gehri seekh kya hai?", [`${seed.focus} tabhi saccha hai jab woh jeevan badalta hai`, "Baahari safalta hi sab kuch hai", "Aatma prashansa par nirbhar hai", "Krodh hi gyaan hai"], 0, "Gita ki gehri seekh vyavahar aur drishti dono badalti hai."),
    q("Yeh chapter Karma Yoga se kaise judta hai?", ["Karme ke peeche ke bhaav ko shuddh karke", "Saare karma ko hamesha ke liye nakaakar", "Ahankar ko result ka maalik banaakar", "Kartavya ko bekaar banaakar"], 0, "Har chapter karma ko gyaan aur bhakti se jodta hai."),
    ...
    q("Kaun sa vakya is chapter ko sabse achha samjhata hai?", [`${seed.remembrance} aur ${seed.practice} ko jeevan mein laana`, "Aadhyaatmikta ka arth nishkriyata hai", "Shareer hi poora aatm hai", "Ichha har nirnay chalaye"], 0, "Chapter ki shiksha yaad, abhyas aur aacharan mein poori hoti hai."),
  ];
...
function makeChapter(seed: HinglishSeed): Chapter {
  const examples: PracticalExample[] = [
    `Padhai ya kaam karte samay ${seed.practice} ko yaad rakhein aur result ki chinta kam karein.`,
  ...
  const intro = [
    `${seed.title} Bhagavad Gita ka mahattvapurn chapter hai. Iska mukhya vishay ${seed.focus} hai. Krishn Arjun ko aasan lekin gehri drishti dete hain, taaki woh kewal bhaavnaon se nahi, balki gyaan aur shraddha se nirnay le sake.`,
    `Is chapter mein ${seed.setting} ko aadhar banaakar jeevan ki disha samjhayi gayi hai. Krishn batate hain ki mannushya ke karma, vichar aur bhaav tabhi shuddh hote hain jab woh ahankar se hattkar satya aur Ishwar se judte hain.`,
    `Aaj ke jeevan mein bhi yeh chapter bahut upyogi hai. Padhai, career, parivaar, rishton aur kathin nirnayon mein ${seed.practice} humein shant, spasht aur zimmedar banata hai.`,
  ].join("\n\n");
  ...
  const storyOverview = [
    ...
    `Krishn is chapter mein samjhate hain ki ${seed.teaching}. Jab mannushya is baat ko bhool jaata hai, tab ${seed.warning} use bhrmit kar deta hai.`,
    `Chapter dheere-dheere yeh dikhata hai ki baahari paristhiti se adhik mahattvapurn bheetar ka bhaav hai. Ek hi karma bandhan bhi ban sakta hai aur mukti ka saadhan bhi, yeh is par nirbhar hai ki woh kis bhaavna se kiya gaya hai.`,
    ...
    `Ant mein chapter sadhak ko ${seed.result} ki or le jaata hai. Yeh seekh kewal Arjun ke liye nahi, har us vyakti ke liye hai jo jeevan ko adhik saccha aur shant banana chahta hai.`,
  ].join("\n\n");

  const mainTeachings: MainTeaching[] = [
    {
      heading: seed.focus,
      body: `Is chapter ki pehli seekh hai ki ${seed.focus} jeevan ki disha badal sakta hai. Jab mannushya ise samajhta hai, tab woh paristhiti se bhaagta nahi, balki sahi drishti se uska saamna karta hai.`,
    },
    {
  ...
  return {
    ...
    lessonsForDailyLife: [
      ...
      `Result se pehle bhaav aur prayas ko shuddh karein.`,
    ...
    reflectionQuestions: [
      `Is chapter ki kaun si seekh aapke jeevan mein sabse adhik lagoo hoti hai?`,
    ...
    seo: {
      metaTitle: `Bhagavad Gita Chapter ${seed.number} Saar - ${seed.title}`,
      metaDescription: `Bhagavad Gita chapter ${seed.number} ka aasan Hinglish saar: ${seed.focus}, daily jeevan ki seekh aur 25 sawalon ka quiz.`,
      keywords: [
        `Bhagavad Gita Chapter ${seed.number}`,
        seed.title,
        seed.sanskritName,
...
const seeds: HinglishSeed[] = [
  ...
  { number: 5, slug: "the-yoga-of-renunciation", title: "Tyaag ka Yoga", sanskritName: "Karma Sanyasa Yoga", subtitle: "Karma karte hue ahankar chhodna", focus: "Bheetar ka saccha tyaag", setting: "Karma aur sannyaas ki tulna", teaching: "Saccha tyaag result ki aasakti chhodna hai", practice: "Kartavya karte hue swamitva chhodna", warning: "Baahari tyaag lekin bheetar aasakti", result: "Bheetar ki shanti", remembrance: "Kamal ki tarah sansaar mein rehkar bhi usse lipt na hon" },
